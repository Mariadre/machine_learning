...
def _eval(sess, top_k_op, input_image, label_placeholder):
    ...
    for index in range(10000):
        ...
        predictions = sess.run([top_k_op],
                               feed_dict={'input_image:0': image.byte_array,
...
def main(argv=None):
    ...
    # eval 用のエントリポイント
    input_image = tf.placeholder_with_default(distorted_image, [32, 32, 3], name='input_image')

    standardized_image = tf.image.per_image_standardization(input_image)

    # (height, weight, depth) -> (batch, height, width, depth)
    image_node = tf.expand_dims(standardized_image, 0)
